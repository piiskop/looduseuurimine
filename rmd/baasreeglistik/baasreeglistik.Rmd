...
\begin{align}
u_\mathrm{B} = \frac{\qty{`r as.character(signif(x = caliper_smallest_unit, digits = 1))`}{\m}}{\num{2}} = \qty{`r as.character(signif(x = caliper_b, digits = 2))`}{\m}.
...
\begin{align}
u = \sqrt{u_\mathrm{A}^2 + u_\mathrm{B}^2} = \sqrt{(\qty{`r as.character(signif(x = caliper_sd_mean, digits = 4))`}{\m})^2 + (\qty{`r as.character(signif(x = caliper_b, digits = 2))`}{\m})^2} = \qty{`r as.character(signif(x = caliper_uncertainty, digits = 2))`}{\m}.
...
Suhteline mõõtemääramatus on $\frac{\qty{`r as.character(signif(x = caliper_sd_mean, digits = 2))`}{\m}}{\qty{`r as.character(signif(x = caliper_mean, digits = 3))`}{\m}} = \num{`r as.character(signif(x = caliper_relative, digits = 3))`}$.
...
illuminance_uncertainty_B_1 <- probability * (illuminance_mean * precision_ratio + number_of_least_sigfig)

illuminance_smallest_unit <- 1
student_inf <- quantile(StudentsT(df = Inf), alpha)
illuminance_uncertainty_B_2 <- student_inf * illuminance_smallest_unit / 2
...
Mõõtetulemuste aritmeetiline keskmine on \qty{`r as.character(illuminance_mean)`}{\m}. A-tüüpi mõõtemääramatus nelja mõõtmise ja \num{95}-\% tõenäosuse korral on \qty{`r as.character(signif(x = illuminance_uncertainty_A, digits = 3))`}{\lux}.
...
\begin{align}
u_\mathrm{B1} &= p \cdot (\text{täpsusosamäär} \cdot \bar{q} + \text{vähim oluliste tüvenumbrite arv})\\
&= \num{`r as.character(probability)`} \times (\num{`r as.character(precision_ratio)`} \times \qty{`r as.character(illuminance_mean)`}{\lux} + \qty{`r as.character(number_of_least_sigfig)`}{\lux}) = \qty{`r as.character(signif(x = illuminance_uncertainty_B_1, digits = 2))`}{\lux}.
...
\begin{align}
u_\mathrm{B2} = t_{\infty, p} \cdot \frac{\text{väikseim ühik}}{2} = \num{`r as.character(signif(x = student_inf, digits = 4))`} \times \frac{\qty{`r as.character(illuminance_smallest_unit)`}{\lux}}{2} = \qty{`r as.character(signif(x = illuminance_uncertainty_B_2, digits = 3))`}{\lux}.
(\#eq:illuminance-uncertainty-B-2)
\end{align}

Kogu mõõtemääramatuse arvutamiseks tuleb kõik alammõõtemääramatuste vektorid kokku liita nagu [nihiku](#caliper-uncertainty) osas. Seega on kogu mõõtemääramatus \qty{`r as.character(signif(x = illuminance_uncertainty, digits = 3))`}{\lux} ja mõõtetulemuse võib välja kirjutada järgnevalt: \num{`r as.character(probability)`}-\% tõenäosusega nelja mõõtmise korral mõõtsin valgustugevuseks \qty{`r as.character(signif(x = illuminance_mean, digits = 3))`(`r as.character(signif(x = illuminance_uncertainty, digits = 3))`)}{\lux}. Suhteline mõõtemääramatus on samadel tingimustel \num{`r as.character(signif(x = illuminance_uncertainty_relative, digits = 3))`}. Viimane näitab, et mõõtmine on ebaõnnestunud ja see tuleks uuesti läbi viia, sest suhteline viga ületab \qty{10}{\percent}.
