...
```{=tex}
\begin{equation}
\overline{x} \pm \mathrm{t}(n - 1, \frac{\alpha}{2}) \times \sqrt{\frac{\sum_{i = 1}^n{(\overline{x} - x_i)^2}}{n \times (n - 1)}}
...
Arvutan A-tüüpi mõõtemääramatuse kasutades valemit \@ref(eq:confidence-interval) leheküljel `r render_pageref(reference = "eq:confidence-interval")`.
