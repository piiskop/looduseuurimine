...
caliper_sd_over_number <- caliper_sd / sqrt(caliper_number)
librarian::shelf(c(
  "distributions3"
))
...
student_8 <- quantile(StudentsT(df = 8), alpha)
caliper_sd_mean <- caliper_sd_over_number * caliper_number_student

caliper_smallest_unit <- .01e-3
caliper_b <- caliper_smallest_unit / 2
#caliper_b_last <- caliper_b * 10
caliper_uncertainty <- caliper_sd_mean + caliper_b
...
Summa tuleb jagada ühega vähem kui mõõtmiste arv, et saada üksiktulemuse dispersioon (avaldis \@ref(eq:caliper-mean-x-squared-sum-over-n-1) leheküljel `r render_pageref(reference = "eq:caliper-mean-x-squared-sum-over-n-1")`).

\begin{align}
\frac{\sum_{i = 1}^n{(\overline{x} - x_i)^2}}{(n - 1)} = \frac{\qty{`r as.character(signif(x = caliper_difference_from_mean_squared_sum, digits = 4))`}{\m\squared}}{\num{`r as.character(caliper_number)`} - \num{1}} = \qty{`r as.character(signif(x = caliper_variance, digits = 4))`}{\m\squared}.
...
\begin{align}
\sqrt{\frac{\sum_{i = 1}^n{(\overline{x} - x_i)^2}}{(n - 1)}} = \sqrt{\qty{`r as.character(signif(x = caliper_variance, digits = 4))`}{\m\squared}} = \qty{`r as.character(signif(x = caliper_sd, digits = 4))`}{\m}.
...
\begin{align}
\sqrt{\frac{\sum_{i = 1}^n{(\overline{x} - x_i)^2}}{n \times (n - 1)}} = \frac{\qty{`r as.character(signif(x = caliper_sd, digits = 4))`}{\m}}{\sqrt{\num{`r as.character(caliper_number)`}}} =\qty{`r as.character(signif(x = caliper_sd_over_number, digits = 4))`}{\m}.
(\#eq:caliper-mean-x-squared-sum-over-n-1-sqrt-over-n-sqrt)
\end{align}

Tulemus on vaja korrutada Studenti jaotuse teguriga *t*, milleks on  \num{0,95} tõenäosuse ja nelja katse korral \num{`r as.character(signif(x = caliper_number_student, digits = 3))`} ning kaheksa katse korral nagu õpilasel \num{`r as.character(signif(x = student_8, digits = 3))`} (avaldis \@ref(eq:caliper-uncertainty-a) leheküljel `r render_pageref(reference = "eq:caliper-uncertainty-a")`).
...
Järgnevalt tuleb mõõtemääramatused liita (avaldis \@ref(eq:caliper-uncertainty) leheküljel `r render_pageref(reference = "eq:caliper-uncertainty")`).

\begin{align}
u = u_\mathrm{A} + u_\mathrm{B} = \qty{`r as.character(signif(x = caliper_sd_mean, digits = 4))`}{\m} + \qty{`r as.character(signif(x = caliper_b, digits = 2))`}{\m} = \qty{`r as.character(signif(x = caliper_uncertainty, digits = 2))`}{\m}.
...
Joonisel \@ref(fig:caliper-uncertainties) leheküljel `r render_pageref(reference = "fig:caliper-uncertainties")` on esitatud mõõdetud keskväärtus koos mõõtemääramatusega graafiliselt.
...
ggplot(data = caliper_data_frame, mapping = aes(x = `caliper`, y = 0)) +                                                                                                                                                                                                   # 13
  ...
  geom_point(aes(x = caliper_mean, y = 0), shape = 23, size = 2) +
  labs(x = TeX("$\\frac{\\bar{\\textit{x}}}{m}$")) +                                                                                                                                                                                              # 15
  geom_errorbarh(aes(xmin = caliper_lower, xmax = caliper_upper), color = color_x) +                                                                                                                                                                      # 16
  theme(axis.title.x = element_text(colour = color_x), axis.text.x = element_text(colour = color_x), axis.ticks.x = element_line(colour = color_x), axis.title.y=element_blank(), 
        axis.text.y=element_blank(), 
        axis.ticks.y=element_blank()) #+
#  scale_x_continuous(
...
Meil on kasutada digitaalne valgusmõõtur sarjast *ST-1300*. Kuna see on digitaalne, tuleb mõõtmisi korrata ja arvutada A-tüüpi mõõtemääramatus. Valgusmõõturiga korratakse mõõtmisi niimoodi, et mõõdetakse samas kohas, kuid vahepeal nullitakse näit kattes sensori. A-tüüpi mõõtemääramatuse arvutamine on kirjeldatud [nihiku](#caliper) sektsioonis.

Kuna selle masina puhul on kirjas ka tootjapoolne tabavus, siis tuleb arvutada B-tüüpi mõõtemääramatus selle järgi. Esitatud on kaks tabavuskarakteristikut: $\pm \num{5} \% \pm \num{10} \text{vähim olulist tüvenumbrit}$, kui valgustus on väiksem kui \qty{10000}{\lux}, ja $\pm \num{10} \% \pm \num{10} \text{vähim olulist tüvenumbrit}$, kui valgustus on suurem kui \qty{10000}{\lux}.
...
Kuna kõik mõõdetud väärtused on väiksemad kui \qty{10000}{\lux}, siis kasutan tabavuskarakteristikut $\pm \num{5} \% \pm \num{10} \text{vähim olulist tüvenumbrit}$.
